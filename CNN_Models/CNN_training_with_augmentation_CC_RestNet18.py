...
RESULTS_DIR = "RESULTS/CC_ResNet18_with_Augmentation"
        ...
        for phase in ["train", "val"]:
            ...
            if phase == "val":
                if epoch_loss < best_val_loss:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(RESULTS_DIR, "best_cc_resnet18_with_aug.pth")
                    )
...
def plot_training_curves(history):
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("CC ResNet18 With Augmentation - Loss")
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.title("CC ResNet18 With Augmentation - Accuracy")
...
def save_confusion_matrix(labels, preds):
    ...
    plt.title("Confusion Matrix - CC ResNet18 With Augmentation")
...
def main():
    ...
    model = CCResNet18(num_classes=2).to(DEVICE)

    # CC train is almost balanced: Benign 242 / Malignant 252
    ...
    with open(os.path.join(RESULTS_DIR, "classification_report.txt"), "w", encoding="utf-8") as f:
        f.write("CC-only ResNet18 With Augmentation\n")
        f.write("==================================\n\n")
        f.write(f"Seed: {SEED}\n")
