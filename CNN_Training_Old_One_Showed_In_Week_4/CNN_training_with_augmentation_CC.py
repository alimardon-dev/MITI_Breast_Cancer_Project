...
# PATHS
# =========================
DATA_DIR = r"C:\Users\USER\Desktop\CNN_training_breast_cancer\cnn_data"

TRAIN_DIR = os.path.join(DATA_DIR, "train", "CC")
VAL_DIR   = os.path.join(DATA_DIR, "val", "CC")
TEST_DIR  = os.path.join(DATA_DIR, "test", "CC")

OUTPUT_DIR = r"C:\Users\USER\Desktop\CNN_training_breast_cancer\results_cc_with_augmentation"
...
# unfreeze layer3 and layer4 for stronger learning
...
# LOSS / OPTIMIZER
# =========================
# class order: ['Benign', 'Malignant']
# stronger malignant penalty
...
def evaluate(loader, threshold=None):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            y_prob_malignant.extend(malignant_probs.cpu().numpy())
...
best_val_acc = 0
best_model_path = os.path.join(OUTPUT_DIR, "best_cc_malignant_focus_model.pth")
...
plt.title("Confusion Matrix - CC Malignant Focus")
...
plt.ylabel("Accuracy")
plt.legend()
plt.title("Accuracy Curve - CC Malignant Focus")
...
plt.ylabel("Loss")
plt.legend()
plt.title("Loss Curve - CC Malignant Focus")
